# AGENTS.md — ADS-B Receiver Desktop App
...
├── AGENTS.md             # Developer instructions and guide (this file)
...
* **SBS-1 records are built from a 22-slot list, never an f-string.** `format_sbs()` fills a `[""] * SBS_FIELD_COUNT` array through the named `IDX_*` constants. The hand-written f-strings this replaced were one field too long and put the alert/emergency/spi/is_on_ground flags at 19–22 instead of 18–21; the columns downstream actually reads happened to be right, so nothing caught it for the life of the feed. Adding a field means setting its index, not counting commas.
* **What the decoder knows, the wire must carry.** `vertical_rate` sat in the decoded `res` dict and in the database for months while `format_sbs()` silently dropped it, so the live Socket.IO stream had no climb rate and the dashboard's `Aircraft.vertical_rate` was permanently undefined. It rides on `MSG,4` (field 16) beside speed and track, which is where pyModeS decodes it from. If you add a decoded field, check it reaches a slot here too — the database path and the rebroadcast path are separate and only one of them is obvious.
