# icao_ranges.py
"""
ICAO 24-bit registration address ranges and country codes.
Translated from tar1090/html/flags.js
"""
...
ICAO_RANGES = [
	...
	{"start": 0xF00000, "end": 0xF07FFF, "country": "ICAO (temporary)", "country_code": None},
	{"start": 0xF09000, "end": 0xF097FF, "country": "ICAO (special use)", "country_code": None},
]

def find_icao_range(icao_hex):
    """
    Given a hex string ICAO address, return a dictionary with keys 'country' and 'country_code'.
    If no range matches, returns default values.
    """
    if not icao_hex:
        return {"country": "Unknown", "country_code": None}
    ...
    for r in ICAO_RANGES:
        if r["start"] <= val <= r["end"]:
            return {"country": r["country"], "country_code": r["country_code"]}
            
    return {"country": "Unassigned", "country_code": None}
